...
public class NPCSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        // Register NPC prefabs with NetworkManager
        if (NetworkManager.Singleton != null && NetworkManager.Singleton.NetworkConfig != null)
        {
            foreach (GameObject npcPrefab in npcPrefabs)
            {
                if (npcPrefab != null)
                {
                    NetworkObject networkObject = npcPrefab.GetComponent<NetworkObject>();
                    if (networkObject != null)
                    {
                        NetworkManager.Singleton.NetworkConfig.Prefabs.Add(new NetworkPrefab { Prefab = npcPrefab });
                        Debug.Log($"Registered NPC prefab: {npcPrefab.name}");
                    }
                    else
                    {
                        Debug.LogError($"NPC prefab {npcPrefab.name} is missing NetworkObject component!");
                    }
                }
            }
        }
        ...
        {
            Debug.LogError("NetworkManager or NetworkConfig not found!");
        }
    }
